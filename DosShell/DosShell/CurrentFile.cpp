...
void editor::loadText()
{
	...
	if (load.is_open())
	{
		...
		x = 0, y = 0;
		s = nullptr; // Initialize s to nullptr

		string str;
		while (getline(load, str)) // Load text into a temporary buffer
		{
			node* lineStart = nullptr; // Initialize lineStart to nullptr

			for (char c : str) // Put character into each line
			{
				t = new node;
				t->c = c;
				t->next = nullptr;

				if (s == nullptr) // First character in the file
				{
					s = t;
					s->prev = nullptr;
					lineStart = s;
			...
			// Create a node for the end of the line
			f = new node;
			f->next = nullptr;
			f->prev = s; // Connect f's prev to the last character in the line
			s->next = f;
			lineStart->prev = f; // Connect lineStart's prev to the end node
			line.push_back(lineStart); // Insert the line into the temporary buffer
			y++;
			x = 0;
			storeFileName(fileName);
		}

		// Append the temporary buffer to the linked list
...
void editor::run()
{
	while ((input = _getch()) != 27) // take user input and exit out if esc is inputed
	{
		...
		case 0:
			...
			break;
		case 'v': // view  List of stored File name
			viewStoredFileNames();
...
void editor::display() // display board
{
	...
	if (menu[0] == "ON")
		cout << "\n     Message:\n     " << message << endl
		<< endl; // display message count if user selete that option
	message = "no Message";
	gotoxy(x + 6, y + 3);
}
int main()
